...
export const splitFrontmatter = (source: string): Result<FrontmatterSplit, ParseFailure> => {
    const lines = source.split('\n');
    if (lines[0] !== '---') {
        return err({
            code: 'unparseable-frontmatter',
            message: 'source MUST begin with a `---` frontmatter fence',
            line: 1,
        });
    }
    ...
    return err({
        code: 'unparseable-frontmatter',
        message: 'frontmatter `---` fence is never closed',
        line: null,
    });
};
...
export const parseMeta = (input: ParseMetaInput): Result<IrMeta, ParseFailure> => {
    ...
    if (id === undefined || id === '' || language === undefined || language === '') {
        return err({
            code: 'unparseable-frontmatter',
            message: 'frontmatter MUST declare a non-empty `id` and `swarm_language`',
            line: null,
        });
    }
