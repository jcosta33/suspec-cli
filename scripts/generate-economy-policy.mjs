...
const source = resolve(explicit ?? join(canon, 'policy', 'economy.md'));
const target = new URL('../src/generated/economyPolicy.ts', import.meta.url);
const targetPath = fileURLToPath(target);

if (!existsSync(source)) {
    if (check && explicit === undefined && process.env.SUSPEC_CANON === undefined) {
        process.stdout.write('economy policy parity skipped: Suspec canon not found\n');
    } else {
        process.stderr.write(`economy policy not found: ${source}\n`);
        process.exitCode = 2;
    }
} else {
    ...
    const raw = [
        ...
        `export const ECONOMY_POLICY_SHA256 = '${digest}';`,
        'export const RECOGNIZED_ECONOMY_POLICIES = new Set([',
        `    '1:${digest}',`,
        ']);',
        `export const ECONOMY_POLICY = ${JSON.stringify(bytes)};`,
        '',
    ].join('\n');
    const output = await format(raw, { ...(await resolveConfig(targetPath)), filepath: targetPath });
    if (check) {
        if (readFileSync(target, 'utf8') !== output) {
            process.stderr.write('generated economy policy is stale\n');
            process.exitCode = 1;
        }
    } else {
