...
8. **Record the run directly in the artifact** — for 1:1 work (no task), fill
   ...
   let the review step route them into `## Candidate findings` for a multi-lens
   review, or `## Human attention` otherwise (see review-output). Findings ride
   the task or review packet; durable ones become native memories (see save-findings).
