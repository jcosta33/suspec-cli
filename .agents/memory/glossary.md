...
to exactly one canonical definition. A term whose meaning is contested MUST be *split* into
...
a run here. Keep entries to one line each. (Suspec working artifacts themselves are transient and
live in the personal store — this file is committed because the terms describe this repo's code.)
