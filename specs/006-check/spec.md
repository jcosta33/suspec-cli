...
status: draft
owner: José Costa
sources: [SPEC-swarm-cli, SPEC-sol-parser, SPEC-sol-lint]
created: 2026-06-08
format: sol
---
...
`specs/*/spec.md` through the `Sol` core module (`parse_spec`), runs the five-layer lint diagnostics
...
THE command MUST discover and parse every `specs/*/spec.md` through the `Sol` module's `parse_spec`, and run the lint diagnostics over each parsed spec
...
REQ AC-005:
WHEN the `specs/` tree contains no `spec.md`
...
CONSTRAINT C-002:
THE command MUST NOT modify, reorder, or rewrite any `specs/*/spec.md`
...
- The v1 contract: discover + parse + lint every `specs/*/spec.md`, aggregate to one repo verdict
