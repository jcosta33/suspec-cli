...
writes a `finding.md` (from `.agents/templates/finding.md`) into `.agents/memory/findings/`, and adds one
...
the `finding.md` shape is the framework's finding template (`.agents/templates/finding.md`); this spec
contracts the command that produces those artifacts, it does not redefine them. The target homes are fixed by
the adoption layout (`AGENTS.md`): findings under `.agents/memory/findings/`, the recall map at
...
THE command MUST write one `finding.md` derived from `.agents/templates/finding.md` into `.agents/memory/findings/`
VERIFY BY test:cmdTest:src/modules/Commands/__tests__/promote.spec.ts#writes_finding_file
DEPENDS ON IF-001
WRITES .agents/memory/findings/**
RISK high
...
CONSTRAINT C-003:
THE command MUST NOT overwrite an existing `finding.md` in `.agents/memory/findings/`
