// Parse the code repo's `.swarm/config.yaml` `agents:` block into adapter records, and resolve the
// adapter a launch should use. PURE (text in, records out) — the file read lives in the use-case.
//
// No YAML dependency: like the rest of swarm-cli (frontmatter is hand-scanned in taskLocator), this
...
export function resolve_adapter(config: AgentConfig, requested?: string): Result<Adapter, AppError> {
    ...
    if (name === null || name === undefined || name.length === 0) {
        return err(
            usage('no agent given and no `agents.default` in .swarm/config.yaml — pass --agent <name>')
        );
    }
    ...
    if (raw === undefined) {
        ...
        return err(usage(`unknown agent "${name}" in .swarm/config.yaml${hint}`));
    }
    if (raw.command === undefined || raw.command.length === 0) {
        return err(usage(`agent "${name}" in .swarm/config.yaml has no \`command\``));
    }
    return ok({
        name,
        command: raw.command,
