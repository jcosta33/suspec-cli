...
// `suspec/<spec-slug>` for a whole spec, `suspec/<spec-slug>/<task-slug>` for one task. Worktrees
...
// (ADR-0046). The ONE shared normalizer, so the PRODUCER (this service, which `suspec worktree create
// --task <t>` drives) and the CONSUMER (resolve_worktree, which review/run drive off the task id) derive
// the same tail: `--task TASK-Feat` and a lookup of `TASK-Feat` both land on `suspec/<spec>/feat`. Without
...
export function derive_worktree_names(input: DeriveWorktreeNamesInput): WorktreeNames {
    ...
    const branch = hasTask ? `suspec/${input.specSlug}/${slug}` : `suspec/${input.specSlug}`;
