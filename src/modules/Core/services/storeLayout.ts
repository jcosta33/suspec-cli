...
// `run-<slug>.md`, `review-<slug>.md`, `finding-<NNN>.md`, `intake-<slug>.md`), evidence under
// `evidence/<run>/`, and `archive/` as the only lifecycle subfolder. No other directory is ever
// derived here — a path this module does not build is a path the store must not contain. Slugs are
// validated by callers via `is_safe_segment` before they reach a builder. Pure.
