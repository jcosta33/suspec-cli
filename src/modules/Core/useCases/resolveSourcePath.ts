...
// puts shared artifacts at the WORKSPACE ROOT — `intake/x.md` (what `swarm pull` writes), `decisions/`,
...
// The workspace root for a spec file when only the file path is known (single-file `swarm check <file>`):
...
// overshoot to an ancestor monorepo (or a workspace nested inside another swarm repo) that also has one.
