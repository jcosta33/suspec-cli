...
export function check_review_file(input: CheckReviewFileInput): Result<CheckReviewFileReport, AppError> {
    ...
    const taskRef = review.task ?? undefined;
    ...
    const specType = parsed.value.frontmatter.type;
    if (specType !== 'spec') {
