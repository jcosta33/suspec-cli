...
const PASTE_PLACEHOLDER = `{{Paste the upstream ticket/PR/page content verbatim here. \`swarm pull\` could
not fetch this ref automatically — only \`gh-issue\`-style refs are fetched via the
\`gh\` CLI today (richer per-tracker connectors are deferred \`swarm-*\` plugins). Do
...
export function pull_intake(input: PullIntakeInput): Result<PullIntakeReport, AppError> {
    ...
    if (ref.length === 0) {
        return err(usage_error('usage: swarm pull <ref> — a ticket ref (a gh issue, a URL, or a tracker key)'));
    }
