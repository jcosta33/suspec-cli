...
describe('agents command (suspec agents emit, ADR-0098)', () => {
    ...
    it('a `status: retired` stub is reported as skipped (not installable), normal agents still emit', async () => {
        writeFileSync(
            join(src, 'suspec-evidence-checker.md'),
            '---\nname: suspec-evidence-checker\nstatus: retired\ndescription: >-\n  Retired — do NOT install.\ntools: Read\n---\n\n# suspec-evidence-checker\n\nRedirect stub.\n'
        );
        const { code, out } = await capture(() => run(['emit', '--from', src], cwd));
        expect(code).toBe(0);
        expect(out).toContain('skipped (status: retired — not installable): suspec-evidence-checker.toml');
        // the retired stub is NOT projected; the normal agent is
        expect(existsSync(join(cwd, '.codex', 'agents', 'suspec-evidence-checker.toml'))).toBe(false);
        expect(existsSync(join(cwd, '.codex', 'agents', 'suspec-reviewer.toml'))).toBe(true);
    });
